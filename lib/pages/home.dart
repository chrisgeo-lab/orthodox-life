...
class _HomeState extends State<Home> {
  ...
  final ScrollController textScrollController = ScrollController();
   ...
   _getData() async{
     if(await webScraper.loadWebPage('/chapel')){
       List<Map<String, dynamic>> getDate = webScraper.getElement('div.commemorate-header > h3',['title']);
       List<Map<String, dynamic>> getFasting = webScraper.getElement('div.oc-fasting p', ['title']);
       List<Map<String, dynamic>> getIcon = webScraper.getElement('div.commemorate-left > div.img-wrapper > img', ['src']);
       List<Map<String, dynamic>> getSaints = webScraper.getElement('ul.oc-saints > li', ['title']);
       List<Map<String, dynamic>> getReadings = webScraper.getElement('ul.oc-readings > li > a', ['href']);
       setState(() {
         now = DateTime.now();
       ...
       if(await webScraper.loadWebPage(epistleAddr.replaceAll(r'http://www.goarch.org', ''))){
         List<Map<String, dynamic>> getSubtitle = webScraper.getElement('section.readings-wrapper > h3', ['title']);
         List<Map<String, dynamic>> getSource = webScraper.getElement('section.readings-wrapper > h1', ['title']);
         List<Map<String, dynamic>> getText = webScraper.getElement('div.details', ['title']);
         setState(() {
       ...
       if(await webScraper.loadWebPage(gospelAddr.replaceAll(r'http://www.goarch.org', ''))){
         List<Map<String, dynamic>> getSubtitle = webScraper.getElement('section.readings-wrapper > h3', ['title']);
         List<Map<String, dynamic>> getSource = webScraper.getElement('section.readings-wrapper > h1', ['title']);
         List<Map<String, dynamic>> getText = webScraper.getElement('div.details', ['title']);
         setState(() {
  ...
  Widget build(BuildContext context) {
    return loaded ? FadeIn(child: display(), duration: Duration(milliseconds: 500)) : Center(
